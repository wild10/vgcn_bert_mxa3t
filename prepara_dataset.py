# ------------------------------------------------------------
# prepare_dataset.py
# re-implementation by wildr.10@gmail.com
# ucsp 2020
# dataset : MEXA3T
#
#
...
if cfg_ds=='sst':
    ...
    train_valid_df = pd.read_csv('data/MEXA3T/train.tsv', encoding='utf-8', header=None, sep='\t')
    ...
    test_df = pd.read_csv('data/MEXA3T/dev.tsv', encoding='utf-8', header=None, sep='\t')
    ...
    idx2label = {0:'0', 1:'1'}
    #train_valid_df = pd.read_csv('data/CoLA/train.tsv', encoding='utf-8', header=None, sep='\t')
    train_valid_df = pd.read_csv('data/MEXA3T/df_train.csv')
    train_valid_df = shuffle(train_valid_df)
    # use dev set as test set, because we can not get the ground true label of the real test set.
    #test_df = pd.read_csv('data/CoLA/dev.tsv', encoding='utf-8', header=None, sep='\t')
    test_df = pd.read_csv('data/MEXA3T/df_test.csv')
    test_df = shuffle(test_df)
    train_valid_size=train_valid_df['Category'].count() # getting labels
    valid_size=int(train_valid_size*valid_data_taux)
    train_size=train_valid_size-valid_size
    test_size=test_df['Category'].count()
    print('CoLA train_valid Total:',train_valid_size,'test Total:',test_size)
    df=pd.concat((train_valid_df,test_df))
    corpus = df['Text'] # getting text tweet comments
    y = df['Category'].values  #y.as_matrix()
    # 获得confidence
    y_prob=np.eye(len(y), len(label2idx))[y]
    corpus_size=len(y)
...
#remove stopwords
'''
Remove stop Spanish words from tweets
'''
