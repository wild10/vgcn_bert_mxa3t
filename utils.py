...
# # # #
# utils.py
# adapted by wildr.10@gmail.com
# source code based on the  @author Zhibin.LU
...
def get_class_count_and_weight(y,n_classes):
    classes_count=[]
    weight=[]
    """ voting scheme for the balance dataset """
    # w = (lambda a, b: torch.tensor([max(a, b)/a, max(a,b)/b]))((torch.tensor(y)==0).sum().float(), (torch.tensor(y)==1).sum().float())
    # weight.append(w[0].item())
    # weight.append(w[1].item())
